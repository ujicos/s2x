...
namespace steam
{
	...
	struct CSteamAPIContext_opaque
	{
		void* interfaces[24];
	};
	...
	struct ContextInitData
	{
		void (*pFn)(void* pCtx);
		uint64_t counter;
		CSteamAPIContext_opaque ctx;
	};

	void* SteamInternal_ContextInit(void* p_context_init_data)
	{
		if (!p_context_init_data)
		{
			return nullptr;
		}

		auto* contextInitData = reinterpret_cast<ContextInitData*>(p_context_init_data);

		if (!contextInitData->pFn)
		{
