...
namespace gsc
{
	inline const std::unordered_map<std::string_view, std::string_view> scr_error_translations
	{
		{ "1823", "Assert fail" },
	...
	inline std::string_view translate_scr_error_format(const char* fmt)
	{
		if (!fmt)
		{
			return "";
		...
		const std::string_view value = fmt;

		if (const auto itr = scr_error_translations.find(value); itr != scr_error_translations.end())
		{
			return itr->second;
		}
